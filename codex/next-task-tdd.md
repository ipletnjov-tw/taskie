---
description: Implement next task using strict TDD (red-green-refactor)
...
After you're done, document your progress with a short summary in `.llm/plans/{current-plan-dir}/task-{current-task-id}.md` and update the status and git commit hash of the subtask(s). Update the task status in `.llm/plans/{current-plan-dir}/tasks.md`.
